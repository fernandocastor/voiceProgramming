...
- Read more than a bit about Natural Language Processing, but there's still a long way to go. (PL) There are some papers that focus on outputs (Stefik 2012), which use "auditories" methods to describe programs by voice. This may be interesting to know "write" programs by voice.  
